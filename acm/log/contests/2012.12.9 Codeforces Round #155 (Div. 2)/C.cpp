...
int main()
{
    ...
    cin>>s>>t;
    map<char,vi> scnt;
    mci tcnt,unchanged,changed;
    si ps;
    int ans=0;
    rep(i,sz(s)) scnt[s[i]].pb(i);
    ft(i,'A','Z') tcnt[i]=0;
    rep(i,sz(t)) tcnt[t[i]]++;
    feach(i,tcnt) if (i->y>sz(scnt[i->x])) ans+=i->y-sz(scnt[i->x]);
    else if (i->y<sz(scnt[i->x])) ps.insert(all(scnt[i->x]));
    feach(i,tcnt)
    {
        if (i->y>sz(scnt[i->x]))
        {
            rep(j,i->y-sz(scnt[i->x]))
            {
                lp
                {
                    char& pc=s[*ps.begin()];
                    if ((i->x<pc||unchanged[pc]==tcnt[pc])&&changed[pc]!=sz(scnt[pc])-tcnt[pc])
                    {
                        changed[pc]++;
                        pc=i->x;
                        ps.erase(ps.begin());
