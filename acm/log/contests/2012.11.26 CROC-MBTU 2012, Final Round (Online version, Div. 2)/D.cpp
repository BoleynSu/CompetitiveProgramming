...
int main()
{
	...
	rep(i,m)
	{
#define calc(j) (L[i]+sqrt(sqr(a-b)+sqr(B[i]-A[j].x))+sqrt(sqr(a)+sqr(A[j].x)))
		int l=0,r=n-1;
		whl(r-l>1)
		{
			...
			db getr=calc(mr);
			if (getl<getr) r=mr-1;
			else if (getl>getr) l=ml+1;
			else l=ml+1,r=mr-1;
		}
		if (cmin(ansf,min(calc(l),calc(r))))
		{
			if (calc(l)<calc(r)) ans=mp(A[l].y,i);
			else ans=mp(A[r].y,i);
		}
		//prt(min(calc(l),calc(r)));
		//cerr<<ans.x+1<<" "<<ans.y+1<<endl;
	}
