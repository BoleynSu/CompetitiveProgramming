...
int main()
{
	...
	cin>>s;
	vi cnt(sz(s)+1);
	rep(i,sz(s)) cnt[i+1]=cnt[i]+(s[i]=='('?+1:-1);
	vvi f(sz(s)+1,vi(sz(s)+1));
	f[0][0]=1;
	ft(i,1,sz(s)) ft(j,0,cnt[i])
	{
		int k=cnt[i]-j;
		if (s[i-1]==')') f[i][j]=f[i-1][j+1]+(cnt[i-1]-(k+1)>=0?f[i-1][cnt[i-1]-(k+1)]:0);
		else f[i][j]=(j-1>=0?f[i-1][j-1]:0)+(cnt[i-1]-(k-1)>=0?f[i-1][cnt[i-1]-(k-1)]:0);
