...
int main()
{
	...
	lli ans;
	{
		prq<pr<lli,int>,vec<pr<lli,int> >,greater<pr<lli,int> > > q;
		vec<lli> d(n,ooll);
		vi rt(n);
		rep(i,n) if (isp[i]) if (cmin(d[i],0ll)) q.push(mp(d[i],i)),rt[i]=i;
		whl(sz(q))
		{
			if (d[q.top().y]==q.top().x)
			{
				...
				rep(i,sz(adj[u]))
				{
					int v=adj[u][i],l=len[u][i];
					if (cmin(d[v],d[u]+l)) q.push(mp(d[v],v)),rt[v]=rt[u];
				}
			}
			q.pop();
		}
		ans=d[0];
		vec<pr<lli,pii> > edges;
		rep(u,n) rep(i,sz(adj[u]))
		{
			int v=adj[u][i],l=len[u][i];
			edges.pb(mp(d[u]+d[v]+l,mp(rt[u],rt[v])));
		}
		srt(edges);
		vi st(n);
		make_set(st);
		rep(i,sz(edges))
		{
			int u=edges[i].y.x,v=edges[i].y.y;
			if (union_set(st,u,v)) ans+=edges[i].x;
		}
	}
	cout<<ans<<endl;
}