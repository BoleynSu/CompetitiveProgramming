	...
	rep(i,height) rep(j,width)
	{
		...
		else
		{
			...
			else if (area[i][j].belong==3-my_player_id)
			{
			    ...
			    int K = attack_factor;
			    int C = (int)(K*B/10+S);
			    area[i][j].value=2.5*area[i][j].bonus+area[i][j].soldier_num;
				area[i][j].need=C;
			}
			else
			{
...
void go()
{
    ...
    for (;;)
    {
    	...
    	if (best==-inf) break;
    	prt(bestneed);
    	prt(hold[beststart.x][beststart.y]);
    ...
    feach(i,attack_num)
        if (i->y==0)
        	q.push(i->x),hold[i->x.x][i->x.y]+=rest,rest=0;
    whl(sz(q))
    {
    	pii u=q.front();
