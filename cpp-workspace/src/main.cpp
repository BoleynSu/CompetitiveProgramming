...
db calc(pdd a,pdd b,pdd c,db r4,db bd,db cd)
{
	pdd C=c+mp(cos(cd)*r4,sin(cd)*r4);
	pdd B=b+mp(cos(bd)*r4,sin(bd)*r4);
	if (dis(B,c)<=r4) rtn dis(a,B)*2;
	if (dis(C,b)<=r4) rtn dis(a,C)*2;
	rtn dis(a,B)+dis(a,C)+dis(B,C);
}
...
int main()
{
	...
	pdd a,b,c;
	a=mp(0,0),b=mp(r1,0);
	db s=sqrt(((r1+r2+r3)/2-r1)*((r1+r2+r3)/2-r2)*((r1+r2+r3)/2-r3)*(r1+r2+r3)/2);
	db h=s/r1;
	db hp=sqrt(sqr(r2)-sqr(h));
	c=mp(hp,h);
	if (r1<=r4&&r2<=r4)
	{
		pdb(3,0)<<endl;
	...
	{
		db ans=+inf;
		pdd ca=a-c,cb=b-c,ba=a-b,bc=c-b;
		db lc=atan2(ca.y,ca.x),rc=atan2(cb.y,cb.x);
		handle(lc,rc);
		db lb=atan2(bc.y,bc.x),rb=atan2(ba.y,ba.x);
		handle(lb,rb);
		db l=lc,r=rc;
		for (db cd=lc;cd<=rc;cd+=pi/1000)
			for (db bd=lb;bd<=rb;bd+=pi/1000)
				cmin(ans,calc(a,b,c,r4,bd,cd));
		pdb(3,ans)<<endl;
	}
//	int n;
//	cin>>n;
...
//	repf(i,2,n+2)
//	{
//		if (dot(p[lst[i-2]]-p[lst[i-1]],p[i]-p[lst[i-1]])>0)
//		{
//			lst.pb(i);
//		}
//		else
