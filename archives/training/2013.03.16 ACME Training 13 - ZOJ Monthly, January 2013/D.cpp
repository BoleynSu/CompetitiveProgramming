...
int n,m,w;
int x[100],y[100],t[100],d[100],z[100];
int s1[100],s2[100];
int tran[100];
bool fnd;
int sum;
...
void dfs(int u)
{
	if (!fnd)
	{
		if (sum)
		{
			rep(i,m) if (z[i]==u) swap(x[i],y[i]);
			if (tran[u]!=-1) dfs(tran[u]);
			else
			{
				sum--;
				rep(i,m) if (x[i]==u&&t[i])
				{
					int v=y[i];
					t[i]--;
					dfs(v);
...
int main()
{
	whl(cin>>n>>m>>w)
	{
		rep(i,m) cin>>x[i]>>y[i]>>t[i]>>d[i]>>z[i];
		rep(i,w) cin>>s1[i]>>s2[i];
		fl(tran,-1);
		rep(i,w) tran[s1[i]]=s2[i];
		fnd=false;
		sum=0;
		rep(i,m) sum+=t[i];
		rep(i,n) dfs(i);
