...
{
	node rt;
	void update(info& upd,const info& get)
	{
		cmax(upd,get);
	}
public:
	...
	void build()
	{
		...
		whl(sz(q))
		{
			...
			{
				if (u==rt) u->n[c]->f=rt;
				else if (u->f->n[c]) u->n[c]->f=u->f->n[c];
				else u->n[c]->f=rt;
				q.push(u->n[c]);
				update(u->n[c]->i,u->n[c]->f->i);
			}
			else u->n[c]=u->f->n[c];
		}
	}
...
int main()
{
	...
	{
		vi lst(sz(s[i]));
		rep(j,sz(lst)) lst[j]=fa[s[i][j]];
		ac.insert(lst,sz(s[i]));
	}
	ac.build();

	vec<vec<map<node,int> > > f(n+1,vec<map<node,int> >(10+1));
	f[0][0][pool]=1;
	rep(i,n)
		ft(j,0,10)
			for(node k=pool;k!=top;k++)
				rep(c,4)
					if (k->n[c])
					{
						if (k->n[c]->i>j)
						{
							f[i+1][0][k->n[c]]+=f[i][j][k];
							f[i+1][0][k->n[c]]%=MOD;
						}
						else if (j<10)
						{
							f[i+1][j+1][k->n[c]]+=f[i][j][k];
							f[i+1][j+1][k->n[c]]%=MOD;
						}
					}
