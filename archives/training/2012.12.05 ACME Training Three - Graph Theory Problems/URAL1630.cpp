...
int main()
{
	...
	ft(i,1,n)
		if (!inq[i])
		{
			...
			bool good=true;
			int cnt1=0;
			rep(i,sz(lst)) if (sz(adj[lst[i]])>2) good=false;
			else if (sz(adj[lst[i]])==1) cnt1++;
			if (good) good=cnt1;

			if (!good&&(sz(lst)==1||sz(lst)==3||sz(lst)==4))
			{
				good=true;
				rep(i,sz(lst)) rep(j,sz(lst)) if (i!=j) if (!count(all(adj[lst[i]]),lst[j])) good=false;
			}
			if (!good) rtn puts("Unlucky Petr"),0;
		}
	puts("Luck is possible");
}