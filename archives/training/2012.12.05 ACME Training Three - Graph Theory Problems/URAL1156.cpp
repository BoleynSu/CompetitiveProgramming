...
int main()
{
	...
	ft(i,1,n*2)
		if (c[i]==-1)
		{
			...
			whl(sz(q))
			{
				int u=q.front();
				q.pop();
				if (c[u]) blk.back().x.pb(u);
				else blk.back().y.pb(u);
				rep(i,sz(adj[u]))
				{
					int v=adj[u][i];
					if (c[v]==-1)
	...
	rep(i,sz(blk))
	{
		if (sz(blk[i].x)>sz(blk[i].y)) swap(blk[i].x,blk[i].y);
		sum+=sz(blk[i].x);
	}
//	rep(i,sz(blk))
//	{
//		cerr<<"X:";
//		rep(j,sz(blk[i].x)) cerr<<blk[i].x[j];
//		cerr<<endl;
//		cerr<<"Y:";
//		rep(j,sz(blk[i].y)) cerr<<blk[i].y[j];
//		cerr<<endl;
//	}
	vvb f(sz(blk)+2,vb(n*2+2));
	vvi g(sz(blk)+2,vi(n*2+2));
	f[0][0]=true;
	ft(i,1,sz(blk))
	{
		int x=sz(blk[i-1].y)-sz(blk[i-1].x);
		ft(j,0,n*2) if (f[i-1][j]) f[i][j]=true,g[i][j]=j;
		ft(j,x,n*2) if (f[i-1][j-x]) f[i][j]=true,g[i][j]=j-x;
	}
	if (!f[sz(blk)][n-sum]) puts("IMPOSSIBLE");
	else
	{
		vi fir(sz(blk));
		int i=sz(blk),j=n-sum;
		whl(i)
		{
			int ii=i-1,jj=g[i][j];
			if (jj==j) fir[i-1]=true;
			else fir[i-1]=false;
			i=ii,j=jj;
		}
		int cnt=0;
		rep(i,sz(blk))
		{
			vi& out=fir[i]?blk[i].x:blk[i].y;
			rep(j,sz(out)) cout<<out[j]<<char(++cnt==n?'\n':' ');
		}
		rep(i,sz(blk))
		{
			vi& out=fir[i]?blk[i].y:blk[i].x;
			rep(j,sz(out)) cout<<out[j]<<char(--cnt==0?'\n':' ');
		}
	}
}
