...
struct reference
{
	vec<pr<int,type> >& array;
	const int index;
	reference(vec<pr<int,type> >& array,int index):array(array),index(index){}
	reference operator=(const type& x)
	{
		bool found=false;
		rep(i,sz(array)) if (array[i].x==index) array[i].y=x,found=true;
		if (!found) array.pb(mp(index,x));
		return reference<type>(array,index);
	}
	type operator->()
	{
		rep(i,sz(array)) if (array[i].x==index) rtn array[i].y;
		rtn 0;
	}
	operator type()
	{
...
{
	vec<pr<int,type> > lst;
	reference<type> operator[](int index)
	{
		rtn reference<type>(lst,index);
	}
};
struct struct_node{array<struct_node*> n;struct_node* f;info i;int v;};
