...
bool my_less(const pii& a,const pii& b)
{
    return a.x<b.x;
}
...
int main()
{
    ...
    ft(t,1,+oo)
    {
        ...
        typedef map<int,mm> stype;
        stype s;
        rep(i,N) s[p[i].x.x].insert(mp(p[i].x.y,p[i].y));
        int M;
        cin>>M;
        rep(i,M)
        {
            ...
            whl(!Q.empty())
            {
                pr<pii,int> qh=Q.front();
                for (stype::itr it=s.lower_bound(qh.x.x-qh.y);it!=s.end()&&it->x<=qh.x.x+qh.y;it->y.size()?it++:(s.erase(it++),it))
                    for (mm::itr jt=it->y.lower_bound(qh.x.y-qh.y);jt!=it->y.end()&&jt->x<=qh.x.y+qh.y;it->y.erase(jt++))
                    {
                        ans++;
                        Q.push(mp(mp(it->x,jt->x),jt->y));
                    }
                Q.pop();
            }
            cout<<ans<<endl;
