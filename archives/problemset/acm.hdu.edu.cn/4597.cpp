...
int a[20],b[20];
int ttl;
bool calced[21][21][21][21];
int f[21][21][21][21];

int dp(int al,int ah,int bl,int bh)
{
    if (cmax(calced[al][ah][bl][bh],true))
    {
        int length=(ah-al)+(bh-bl);
        if (length)
        {
            bool max=length%2==0;
            if (max)
            {
                f[al][ah][bl][bh]=-oo;
                if (al<ah) cmax(f[al][ah][bl][bh],dp(al+1,ah,bl,bh)+a[al]);
                if (al<ah) cmax(f[al][ah][bl][bh],dp(al,ah-1,bl,bh)+a[ah-1]);
                if (bl<bh) cmax(f[al][ah][bl][bh],dp(al,ah,bl+1,bh)+b[bl]);
                if (bl<bh) cmax(f[al][ah][bl][bh],dp(al,ah,bl,bh-1)+b[bh-1]);
            }
            else
            {
                f[al][ah][bl][bh]=+oo;
                if (al<ah) cmin(f[al][ah][bl][bh],dp(al+1,ah,bl,bh)-a[al]);
                if (al<ah) cmin(f[al][ah][bl][bh],dp(al,ah-1,bl,bh)-a[ah-1]);
                if (bl<bh) cmin(f[al][ah][bl][bh],dp(al,ah,bl+1,bh)-b[bl]);
                if (bl<bh) cmin(f[al][ah][bl][bh],dp(al,ah,bl,bh-1)-b[bh-1]);
            }
        }
        else f[al][ah][bl][bh]=0;
    }
    rtn f[al][ah][bl][bh];
}
...
int main()
{
    ...
    whl(T--)
    {
        sf("%d",&n);
        ttl=0;
        rep(i,n) sf("%d",&a[i]),ttl+=a[i];
        rep(i,n) sf("%d",&b[i]),ttl+=b[i];
        clr(calced);
        cout<<(ttl+dp(0,n,0,n))/2<<endl;
    }
}
