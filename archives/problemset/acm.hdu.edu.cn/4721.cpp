...
int lsts;
pii lst[MAXN*MAXN];
int ast[MAXN*MAXN],bst[MAXN*MAXN];
bool cmp(pii a,pii b){rtn a.x-a.y<b.x-b.y;}
inline void bit_inc(int* st,int x,int inc){whl(x<lsts)cmax(st[x],inc),x|=x+1;}
int bit_sum(const int* st,int x){int s=-oo;whl(x>=0)cmax(s,st[x]),x=(x&(x+1))-1;rtn s;}
...
int main()
{
    ...
    ft(t,1,T)
    {
        pf("Case #%d:\n",t);
        sf("%d%d%d%d",&n,&m,&r,&q);
        rep(i,n) rep(j,m) sf("%d",&a[i][j]);
        rep(i,n) rep(j,m) sf("%d",&b[i][j]);
        rep(i,n) rep(j,m)
        {
            sa[i][j]=(i?sa[i-1][j]:0)+(j?sa[i][j-1]:0)-(i&&j?sa[i-1][j-1]:0)+a[i][j];
            sb[i][j]=(i?sb[i-1][j]:0)+(j?sb[i][j-1]:0)-(i&&j?sb[i-1][j-1]:0)+b[i][j];
        }
        lsts=0;
        rep(i,n) rep(j,m)
        {
            lst[lsts++]=mp(sa[i][j]-(i-r*2>0?sa[i-r*2-1][j]:0)-(j-r*2>0?sa[i][j-r*2-1]:0)+(i-r*2>0&&j-r*2>0?sa[i-r*2-1][j-r*2-1]:0),
                            sb[i][j]-(i-r*2>0?sb[i-r*2-1][j]:0)-(j-r*2>0?sb[i][j-r*2-1]:0)+(i-r*2>0&&j-r*2>0?sb[i-r*2-1][j-r*2-1]:0));
        }
        sort(lst,lst+lsts,cmp);
        //rep(i,lsts) prt(lst[i]);
        rep(i,lsts) ast[i]=bst[i]=0;
        rep(i,lsts)
        {
            bit_inc(ast,i,lst[i].x);
            bit_inc(bst,i,lst[lsts-1-i].y);
        }
        rep(i,q)
        {
            int A,B;
            sf("%d%d",&A,&B);
            int ai=ub(lst,lst+lsts,mp(B,A),cmp)-lst-1;
            int bi=lsts-1-(lb(lst,lst+lsts,mp(B,A),cmp)-lst);
            prt(ai),prt(bi);
            pf("%d\n",max(bit_sum(ast,ai)+A,bit_sum(bst,bi)+B));
        }
        if (t!=T) puts("");
    }
}
