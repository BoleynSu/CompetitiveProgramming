...
void dfs(int x,int ttl=0)
{
    if (x<n)
    {
        rep(i,1<<m)
        {
            st[x]=i;
            bool chk=true;
            if (st[x]&msk[x]) chk=false;
            rep(y,m) if (st[x]&(1<<y))
                rrepf(mx,x,0) if ((st[mx]|msk[mx])&(1<<y))
                {
                    rrepf(lx,mx,0) if ((st[lx]|msk[lx])&(1<<y))
                    {
                        if (st[lx]&(1<<y)) chk=false;
                        break;
                    }
                    break;
                }
            rrepf(ry,m,0) if (st[x]&(1<<ry))
            {
                rrepf(my,ry,0) if ((st[x]|msk[x])&(1<<my))
                {
                    rrepf(ly,my,0) if ((st[x]|msk[x])&(1<<ly))
                    {
                        if (st[x]&(1<<ly)) chk=false;
                        break;
                    }
                    break;
                }
