...
void dfs(map<pii,int>::itr cur,int get,map<pii,int>& p,int& ans)
{
    cmax(ans,get*4);
    for (map<pii,int>::itr i=cur;i!=p.end();i++) if (i->y>0)
    {
        map<pii,int>::itr b=i;
        map<pii,int>::itr e=++b;
        whl(e!=p.end()&&e->x.x==i->x.x) e++;
        for (map<pii,int>::itr j=b;j!=e;j++) if (j->y>0)
        {
            pii a=i->x,b=j->x;
            pii c=a+mp(b.y-a.y,0),d=b+mp(b.y-a.y,0);
            if (p.count(c)&&p.count(d)&&p[c]>0&&p[d]>0)
            {
                i->y--,j->y--;
                p[c]--,p[d]--;
                dfs(i,get+1,p,ans);
    ...
    if (++cur!=p.end()) dfs(cur,get,p,ans);
}
