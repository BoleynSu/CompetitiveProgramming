...
int main()
{
    ...
    whl(T--)
    {
        int n;
        cin>>n;
        vec<pr<db,pdd> > lst(n+1);
        cin>>lst;
        int ans=0;
        rrep(i,n+1) lst[i].y-=lst[0].y;
        ft(i,1,n) if (lst[i].x>lst[0].x||!sgn(sqrt(sqr(lst[i].y.x)+sqr(lst[i].y.y)))) ans=1;
        if (ans)
        {
            cout<<ans<<endl;
            continue;
        ...
        ft(i,1,n)
        {
            db d=acos(sqrt(sqr(lst[0].x)-sqr(lst[i].x))/lst[0].x);
            db b=atan2(lst[i].y.y,lst[i].y.x);
            rng.pb(mp(b-d,b+d));
        }
        rep(i,n)
        {
            whl(rng[i].x<0) rng[i]+=mp(pi*2,pi*2);
            whl(rng[i].x>=pi*2) rng[i]-=mp(pi*2,pi*2);
        }
        srt(rng);
        ans=n+1;
        rep(i,n)
        {
            rng.pb(rng[i]+mp(pi*2,pi*2));
            db last=rng[i].y,end=rng[i].x+pi*2;
            int cnt=1;
            repf(j,i+1,i+n)
            {
                int k=j;
                db nlast=last;
                whl(k<n+i&&dbcmp(last,rng[k].x)>=0) cmax(nlast,rng[k].y),k++;
                if (k==j) break;
                j=k-1;
                last=nlast;
                cnt++;
                if (dbcmp(last,end)>=0) break;
            }
            if (dbcmp(last,end)>=0) cmin(ans,cnt);
        }
        cout<<ans%(n+1)<<endl;
    }
}
