...
const int MAIN_STACK=14;
char static_stack[52*1024*1024];
...
pii dfs(int u,int p)
{
    ...
    rep(i,sz(adj[u]))
    {
        ...
        {
            pii get=dfs(v,u);
            if (get.y-get.x<c1.y-c1.x)
            {
                c2=c1;
                c1=get;
            }
            else if (get.y-get.x<c2.y-c2.x)
            {
                c2=get;
            }
            ttl+=get.x;
    ...
    pii ans=mp(+oo,+oo);
    cmin(ans.y,ttl+c1.y-c1.x);
    cmin(ans.x,ttl+c1.y-c1.x+c2.y-c2.x+1);
    cmin(ans.x,ans.y+1);
    rtn ans;
}
...
int main()
{
    __asm__("movl %%esp, (%%eax);\n"::"a"(static_stack):"memory");
    __asm__("movl %%eax, %%esp;\n"::"a"(static_stack+sizeof static_stack-MAIN_STACK):"%esp");
    int T;
    sf("%d",&T);
    whl(T--)
    {
