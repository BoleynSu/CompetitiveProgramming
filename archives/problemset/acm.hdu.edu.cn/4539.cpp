...
bool mainf()
{
    ...
    int sc[169];
    int ans=0;
    static int f[100][169][169];
    //static int g[100][169][169];
    int ms[100];
    clr(f),clr(ms);
    if (cin>>n>>m) ;
    else return false;
    ...
    rep(i,p2)
    {
        ...
        rep(k,m) if (i&(1<<k))
        {
            if ((i&(1<<(k-2)))||(i&(1<<(k+2)))) pb=false;
            c++;
        }
        if (pb) s[ss]=i,sc[ss]=c,ss++;
    }
    rep(i,n) rep(j,m) ms[i]<<=1,ms[i]|=map[i][j]==0;
    rep(i,ss) if (!(s[i]&ms[0])) f[0][i][0]=sc[i];

    int kss[169];
    int ks[169][169];
    clr(kss);
    rep(j,ss) rep(k,ss) if (!(s[j]&(s[k]<<1))&&!(s[j]&(s[k]>>1))) ks[j][kss[j]++]=k;
    repf(i,1,n) rep(j,ss) if (!(s[j]&ms[i])) rep(k,kss[j]) rep(l,ss)
        if (!(s[j]&s[l]))
            cmax(f[i][j][ks[j][k]],f[i-1][ks[j][k]][l]+sc[j]);
    if (n) rep(i,ss) rep(j,ss) cmax(ans,f[n-1][i][j]);
    cout<<ans<<endl;
    return true;
}
