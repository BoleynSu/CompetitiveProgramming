...
void dfs(vvi& adj,vvi& qry,int u,int p,vi& dep,vi& st,vb& ins,int& ans)
{
    dep[u]=p==-1?0:dep[p]+1;
    rep(i,sz(adj[u]))
    {
        int v=adj[u][i];
        if (v!=p) dfs(adj,qry,v,u,dep,st,ins,ans);
    }
    int delta=0;
    rep(i,sz(qry[u]))
    {
        ...
        if (dep[v]>dep[u])
        {
            if (union_set(st,v,sz(adj))) delta++;
        }
        else ins[v]=true;
    }
    rep(i,sz(adj[u]))
    {
        int v=adj[u][i];
        if (v!=p&&find_set(st,v)!=find_set(st,sz(adj))) union_set(st,v,u);
    }
    ans+=delta;
}
...
int main()
{
    ...
    whl(cin>>n>>m,n||m)
    {
        ...
        vb ins(n);
        make_set(st);
        dfs(tadj,gadj,0,-1,dep,st,ins,ans);
        cout<<ans<<endl;
    }
}
