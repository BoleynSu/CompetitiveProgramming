...
int main()
{
	...
	ft(i,1,n)
		if (c[i]==-1)
		{
			...
			whl(sz(q))
			{
				...
				{
					int v=adj[u][i];
					if (!inlst[v]&&c[v]!=-1&&c[v]!=!c[u]) inlst[u]=true;
				}
				if (inlst[u]) lst.pb(u);
				else
					rep(i,sz(adj[u]))
	...
	rep(i,sz(lst))
	{
		...
		int min=+oo;
		rep(i,2) if (cmin(min,cnt[i])) c[u]=i;
		inlst[u]=false;
	}
	vi ans;
	ft(i,1,n) if (c[i]) ans.pb(i);
	if (sz(ans)*2==n)
	{
		if (!c[1])
		{
	...
	cout<<sz(ans)<<endl;
	rep(i,sz(ans)) cout<<ans[i]<<char(i+1==sz(ans)?'\n':' ');
}
