...
int sum[1000000],cs[1000000];
int ans,ansi,ansj;

int main()
{
	scanf("%d%d%d",&n,&m,&k);
	ft(i,1,n) scanf("%d%d",w+i,c+i),sum[i]=sum[i-1]+w[i],cs[i]=cs[i-1]+c[i];
	fdt(i,n,m+1)
	{
		int j=i-m;
		int l=0,r=i-m;
		...
		{
			int mid=(l+r)>>1;
			if ((sum[j]-sum[mid])*k<sum[i]-sum[j]) r=mid;
			else l=mid;
		}
		if ((sum[j]-sum[l])*k<sum[i]-sum[j]) break;
		if (cmax(ans,cs[n]-(cs[i]-cs[l]))) ansi=l,ansj=i;
	}
	if (ans==0)
	{
		cout<<0<<" "<<0<<endl;
