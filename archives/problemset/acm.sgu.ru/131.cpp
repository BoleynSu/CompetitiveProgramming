...
int main()
{
	...
	ft(i,1,m)
	{
		clr(g);
		rep(s,1<<n) g[s]=f[s];
		repf(s1,1,1<<n) rep(s2,1<<n)
		{
			int i;
			rep(j,n) if (s1&(1<<j)) i=j;
			if ((s1&(1<<i))&&(s2&(1<<i))) g[s1<<n|s2]+=g[(s1^(1<<i))<<n|(s2^(1<<i))];
			if (i&&(s1&(1<<i))&&(s1&(1<<(i-1))))
			{
				g[s1<<n|s2]+=g[(s1^(1<<i)^(1<<(i-1)))<<n|s2];
				if (s2&(1<<i)) g[s1<<n|s2]+=g[(s1^(1<<i)^(1<<(i-1)))<<n|(s2^(1<<i))];
				if (s2&(1<<(i-1))) g[s1<<n|s2]+=g[(s1^(1<<i)^(1<<(i-1)))<<n|(s2^(1<<(i-1)))];
			}
			if (i&&(s2&(1<<i))&&(s2&(1<<(i-1)))&&(s1&(1<<i)))
				g[s1<<n|s2]+=g[(s1^(1<<i))<<n|(s2^(1<<i)^(1<<(i-1)))];
			if (i+1<n&&(s2&(1<<i))&&(s2&(1<<(i+1)))&&(s1&(1<<i)))
				g[s1<<n|s2]+=g[(s1^(1<<i))<<n|(s2^(1<<i)^(1<<(i+1)))];
		}
		rep(s,1<<n) f[s]=g[s<<n|((1<<n)-1)];
	}
	cout<<f[(1<<n)-1]<<endl;
}
