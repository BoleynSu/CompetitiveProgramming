...
{
	scanf("%d",&n);
	ft(i,2,n) scanf(" %c%d",&type[i],&number[i]);
	typedef multiset<pii> stype;
	stype s;
	ft(i,2,n-1)
	{
		if (type[i]=='d') s.insert(pii(number[i],i));
		else
		{
			int x=number[i];
			whl(sz(s)>=x) s.erase(s.begin());
		}
	}
	if (sz(s)<number[n]) puts("-1");
	else
	{
		lli sum=0;
		feach(i,s,stype) sum+=i->first;
		cout<<sum<<endl;
		cout<<sz(s)<<endl;
		vi lst;
		feach(i,s,stype) lst.push_back(i->second);
		srt(lst);
		rep(i,sz(lst)) cout<<lst[i]<<char(i+1==sz(lst)?'\n':' ');
	}
}
