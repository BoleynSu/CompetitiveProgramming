...
int main()
{
	...
	ft(i,1,n)
	{
		whl(ai<i&&a[ai]<b[i]) ai++;
		whl(bi<i&&b[bi]<a[i]) bi++;
		ft(j,0,n) f[i][j]=((j-(a[i]>b[i])>=0?f[i-1][j-(a[i]>b[i])]:0)+f[i-1][j-1]*(i-ai+i-j-(i-bi))+f[i-1][j]*(i-(i-ai+i-j-(i-bi))))%MOD;
	}
	if ((n-k)%2==1) puts("0");
	else printf("%d\n",int(f[n][(n-k)/2+k]));
}