...
void dfs(const vvi& adj,map<pii,int>& clr,int u,int cclr)
{
	rep(i,sz(adj[u]))
	{
		int v=adj[u][i];
		if (!clr.count(mp(u,v)))
		{
			clr[mp(u,v)]=clr[mp(v,u)]=cclr;
			//cout<<u<<"->"<<v<<":"<<cclr<<endl;
			dfs(adj,clr,v,cclr=3-cclr);
		}
	}
}
...
int main()
{
	...
	rep(u,n)
	{
		int v;
		whl(cin>>v,v--)
			adj[u].pb(v),edg.pb(mp(u,v));
	}
	rep(u,n) if (sz(adj[u])%2==1) dfs(adj,clr,u,1);
	rep(u,n) dfs(adj,clr,u,1);
	rep(u,n) if (sz(adj[u])>1)
	{
		si s;
		rep(i,sz(adj[u])) s.insert(clr[mp(u,adj[u][i])]);
		if (sz(s)<2) rtn cout<<"No solution"<<endl,0;
	}
	rep(u,n)
	{
		rep(i,sz(adj[u])) cout<<clr[mp(u,adj[u][i])]<<' ';
		cout<<0<<endl;
	}
}
