...
int main()
{
	...
	map<int,vi> x,y;
	rep(i,n) x[ps[i].x].pb(ps[i].y),y[ps[i].y].pb(ps[i].x);

	vec<pr<pii,pii> > xedges,yedges;
	...
	feach(i,x)
	{
		...
		int x=i->x;
		const vi& y=i->y;
		if (sz(y)&1) rtn cout<<0<<endl,0;
		for (int i=0;i<sz(y);i+=2)
		{
			xedges.pb(mp(mp(x,y[i]),mp(x,y[i+1])));
			union_set(st,lb(all(ps),xedges.back().x)-ps.begin(),lb(all(ps),xedges.back().y)-ps.begin());
			ans+=y[i+1]-y[i];
		}
	}
	feach(i,y)
	{
		...
		int y=i->x;
		const vi& x=i->y;
		if (sz(x)&1) rtn cout<<0<<endl,0;
		for (int i=0;i<sz(x);i+=2)
		{
			yedges.pb(mp(mp(x[i],y),mp(x[i+1],y)));
			union_set(st,lb(all(ps),yedges.back().x)-ps.begin(),lb(all(ps),yedges.back().y)-ps.begin());
			ans+=x[i+1]-x[i];
		}
	}

	rep(i,sz(ps)) find_set(st,i);
	if (*max_element(all(st))!=*min_element(all(st))) rtn cout<<0<<endl,0;

	srt(xedges);
	rep(i,sz(yedges))
	{
		int x1=yedges[i].x.x,x2=yedges[i].y.x,y=yedges[i].x.y;
		for (vec<pr<pii,pii> >::itr it=lb(all(xedges),mp(mp(x1,-oo),mp(x1,-oo)));it!=xedges.end()&&it->x.x<=x2;it++)
			if ((it->x.y<y)==(y<it->y.y)) rtn cout<<0<<endl,0;
	}
