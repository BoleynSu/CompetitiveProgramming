...
const lli SEED1=19930309,SEED2=1000000007;
int n,m;
int k;
char s[4][100000];
map<vi,int> cnt;
...
bool operator<(const vi& a,const vi& b)
{
	...
	{
		rep(i,sz(a)) if (a[i]!=b[i]) return a[i]<b[i];
		return false;
	}
}
...
int get_id(cstr s)
{
	msi::itr it=type.find(s);
	if (it==type.end()) it=type.insert(mp(s,sz(type)+1)).first;
	return it->second;
}
void hash_inc()
{
...
int main()
{
	...
	{
		scanf("%d",&k);
		rep(i,k) scanf("%s",s[i]),id[i]=get_id(s[i]);
		hash_inc();
	}
	scanf("%d",&m);
	rep(i,m)
