...
int main()
{
	str s;
	cin>>s;
	vvi f(sz(s),vi(sz(s))),g(sz(s),vi(sz(s)));
	rep(i,sz(s)) repf(j,i,sz(s)) f[j-i][j]=(j-i+1>j-1||f[j-i+1][j-1])&&(s[j-i]==s[j]);
	rep(i,sz(s)) repf(j,i,sz(s)) g[j-i][j]=(j-i+1<sz(s)?g[j-i+1][j]:0)+(j-1>=0?g[j-i][j-1]:0)-(j-i+1<=j-1?g[j-i+1][j-1]:0)+f[j-i][j];
	int q;
	cin>>q;
	rep(i,q)
	{
