...
int main()
{
	...
	cin>>a;
	vec<lli> sum(n+1);
	rep(i,n) sum[i+1]=sum[i]+max(a[i],0);
	map<int,int> fo;
	rep(i,n) if (!fo.count(a[i])) fo[a[i]]=i;
	lli best=-ooll,besti;
	rep(i,n)
	{
		if (fo[a[i]]!=i&&cmax(best,sum[i]-sum[fo[a[i]]+1]+a[i]*2))
			besti=i;
	}
	vi lst;
	rep(i,fo[a[besti]]) lst.pb(i+1);
	repf(i,fo[a[besti]]+1,besti) if (a[i]<0) lst.pb(i+1);
	repf(i,besti+1,n) lst.pb(i+1);
	cout<<best<<" "<<sz(lst)<<endl<<lst<<endl;
}
