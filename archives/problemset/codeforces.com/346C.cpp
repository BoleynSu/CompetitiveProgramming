...
{
	for (int x=max(r-R+1,1);x<=r;x+=x&(-x))
		cmax(st[x],v);
}
int qry(int l,int r,int L,int R)
{
	int ans=-oo;
	for (int x=r-R+1;x;x-=x&(-x))
		cmax(ans,st[x]);
	rtn ans;
}
...
int main()
{
	...
	for (lli i=a;i<=b;i++)
	{
		int fi=-qry(0,b-a,i-a,i-a);
		prt(i);
		prt(fi);
		prt(mp(i+1,i+adj[i-a]-1));
		upd(0,b-a,i+1-a,i+adj[i-a]-1-a,-(fi+1));
	}
	cout<<-qry(0,b-a,b-a,b-a)<<endl;
}
