...
int main()
{
	...
	{
		map<int,vi> f;
		rep(j,m) f[a[i][j]].pb(j);
		map<int,vi>::itr jt=f.end();
		feach(it,f)
		{
			if (it->x!=-1)
			{
				if (jt!=f.end())
				{
					vi& vit=it->y,vjt=jt->y;
					adj.pb(vi()),ind.pb(0);
					rep(k,sz(vjt)) adj[vjt[k]].pb(tp),ind[tp]++;
					rep(k,sz(vit)) adj[tp].pb(vit[k]),ind[vit[k]]++;
					tp++;
				}
				jt=it;
			}
	...
	qi q;
	vi lst;
	rep(i,m) if (ind[i]==0) q.push(i);
	whl(sz(q))
	{
		int u=q.front();
		if (u<m) lst.pb(u);
		q.pop();
		rep(i,sz(adj[u])) if (--ind[adj[u][i]]==0) q.push(adj[u][i]);
	}
	if (sz(lst)<m) cout<<-1<<endl;
	else
	{
		rep(i,m) cout<<lst[i]+1<<char(i+1==m?'\n':' ');
	}
}
