...
typedef int node;
node st[(MAXN<<1)-1];
void insert(int l,int r,int p,int v,int dep)
{
	if (p<l||r<p) ;
	...
	{
		int m=(l+r)>>1;
		insert(l,m,p,v,dep-1),insert(m+1,r,p,v,dep-1);
		if (dep&1) st[rt]=st[lrt]|st[rrt];
		else st[rt]=st[lrt]^st[rrt];
	}
}
...
int main()
{
	...
	rep(i,1<<n)
	{
		int ai;
		cin>>ai;
		insert(0,(1<<n)-1,i,ai,n);
	}
	rep(i,m)
	{
		int p,b;
		cin>>p>>b;
		insert(0,(1<<n)-1,p-1,b,n);
		prt(st[idx(0,1)]);
