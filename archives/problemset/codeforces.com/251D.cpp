...
vi row,col;
vi prow;
bool solve(vvb& m,int r,int n)
{
	vb backup=m[row[r]];
	rep(i,sz(prow)) if (m[row[r]][col[i]]) ft(j,i,n) m[row[r]][col[j]]=m[row[r]][col[j]]^m[prow[i]][col[j]];
	repf(i,sz(prow),n) if (m[row[r]][col[i]]) swap(col[sz(prow)],col[i]);
	if ((!m[row[r]][col[sz(prow)]])&&m[row[r]][col[n]])
	{
		m[row[r]]=backup;
		rtn false;
	}
	...
	{
		if (m[row[r]][col[sz(prow)]]) prow.pb(row[r]);
		rtn true;
	}
}
void solve(vvb& m,int n,vb& ans)
{
	fdt(i,sz(prow)-1,0)
	{
		ans[col[i]]=m[prow[i]][col[n]];
		repf(j,i+1,n) ans[col[i]]=ans[col[i]]^(m[prow[i]][col[j]]&ans[col[j]]);
	}
}
...
int main()
{
	...
	vvb m(MAXBIT,vb(n+1));
	rep(i,n) rep(j,MAXBIT) m[j][i]=(a[i]>>j)&1;

	lli A=0;
	rep(i,n) A^=a[i];
	fdt(i,MAXBIT-1,0) if (!((A>>i)&1)) row.pb(i);
	fdt(i,MAXBIT-1,0) if (((A>>i)&1)) row.pb(i);
	ft(i,0,n) col.pb(i);
