...
int main()
{
	...
	{
		vec<pr<pll,pll> > t;
		rep(j,n) if (i&(1<<j)) t.pb(r[j]);
		lli a=0;
		pr<pll,pll> s=mp(mp(+oo,+oo),mp(-oo,-oo));
		rep(j,sz(t)) a+=(t[j].y-t[j].x).x*(t[j].y-t[j].x).y,cmin(s.x.x,t[j].x.x),cmax(s.y.x,t[j].y.x),cmin(s.x.y,t[j].x.y),cmax(s.y.y,t[j].y.y);
		if (a==(s.y-s.x).x*(s.y-s.x).y&&(s.y-s.x).x==(s.y-s.x).y) rtn cout<<"YES"<<endl,0;
	}
	cout<<"NO"<<endl;
}
