...
{
	int n=4,d=2;
	vpii ps={pii(1,d),pii(d,1),pii(1,n+1-d),pii(d,n),pii(n+1-d,1),pii(n,d),pii(n+1-d,n),pii(n,n+1-d)};
	rep(i,1<<8)
	{
		bool chk=true;
		rep(j,8) if ((i>>j)&1)
			rep(k,8) if ((i>>k)&1)
			{
				int dis=dot(ps[j]-ps[k],ps[j]-ps[k]);
				if (dis==sqr(n-1)||dis==sqr(d-1)*2||dis==sqr(n-d)*2)
					chk=false;
			}
		if (chk) lst.pb(i);
	}
...
int main()
{
	...
	cin>>p;
	si x,y;
	rep(i,m) x.ins(p[i].x),y.ins(p[i].y);
	int ans=0;
	ft(d,2,n/2)
	{
		vpii ps={pii(1,d),pii(d,1),pii(1,n+1-d),pii(d,n),pii(n+1-d,1),pii(n,d),pii(n+1-d,n),pii(n,n+1-d)};
		...
		rep(i,sz(lst))
		{
			...
			rep(j,8) if ((m>>j)&1)
			{
				if (ps[j].x>1&&ps[j].x<n&&x.count(ps[j].x)) chk=false;
				if (ps[j].y>1&&ps[j].y<n&&y.count(ps[j].y)) chk=false;
			}
			if (chk) cmax(best,get);
		}
		ans+=best;
	}
	if ((n&1)&&(n+1)/2<n&&(n+1)/2>1&&(!x.count((n+1)/2)||!y.count((n+1)/2)))
		ans++;
	cout<<ans<<endl;
}
