...
int main()
{
	...
	pr<pii,pii> s;
	rep(i,n) rep(j,m) if (g[i][j]=='S') s=mp(mp(i,j),mp(0,0));
	vec<pii> d;
	d.pb(mp(1,0));
	d.pb(mp(0,1));
	d.pb(mp(0,-1));
	d.pb(mp(-1,0));
	que<pr<pii,pii> > q;
	vec<vec<set<pii> > > inq(n,vec<set<pii> >(m));
	q.push(s);
	inq[s.x.x][s.x.y].insert(s.y);
	whl(sz(q))
	{
		pr<pii,pii> u=q.front();
		q.pop();
		rep(i,sz(d))
		{
			pr<pii,pii> v=mp(mp(u.x.x+d[i].x,u.x.y+d[i].y),u.y);
			if (v.x.x<0) v.x.x+=n,v.y.x--;
			if (v.x.x>=n) v.x.x-=n,v.y.x++;
			if (v.x.y<0) v.x.y+=m,v.y.y--;
			if (v.x.y>=m) v.x.y-=m,v.y.y++;
			if (g[v.x.x][v.x.y]!='#'&&!inq[v.x.x][v.x.y].count(v.y))
			{
				q.push(v);
				inq[v.x.x][v.x.y].insert(v.y);
				if (sz(inq[v.x.x][v.x.y])>1) rtn puts("Yes"),0;
			}
		}
	}
	puts("No");
