...
int main()
{
	...
	rep(i,n) cin>>b[i],B[--b[i]]=i;
	vi Ab(n);
	rep(i,n) Ab[i]=A[b[i]];
	vi Ab_i(n);
	rep(i,n) Ab_i[i]=Ab[i]-i;
	mii l,r;
	rep(i,n) r[Ab_i[i]]++;
	rep(k,n)
	{
		...
		it=l.lower_bound(n-k);
		if (it!=l.end()) cmin(output,it->first-(n-k)),it==l.begin()?it=l.end():it--;
		else if (!l.empty()) it=l.find(l.rbegin()->first);
		if (it!=l.end()) cmin(output,(n-k)-it->first);

		it=r.lower_bound(-k);
		if (it!=r.end()) cmin(output,it->first-(-k)),it==r.begin()?it=r.end():it--;
		else if (!r.empty()) it=r.find(r.rbegin()->first);
		if (it!=r.end()) cmin(output,(-k)-it->first);

		//ft(i,0,k-1) cmin(output,abs(Ab_i[i]-n+k));
		//ft(i,k,n-1) cmin(output,abs(Ab_i[i]+k));
		cout<<output<<endl;
		l[Ab_i[k]]++;
		r[Ab_i[k]]--;
		if (!r[Ab_i[k]]) r.erase(Ab_i[k]);
	}
}
