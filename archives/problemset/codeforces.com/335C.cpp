...
map<pr<pr<pii,pii>,int>,int> sg;

int get(int a,int b,int c,int d,int l)
{
	if (!sg.count(mp(mp(mp(a,b),mp(c,d)),l)))
	{
		int& SG=sg[mp(mp(mp(a,b),mp(c,d)),l)];
		if (l==0) SG=0;
		else if (l==1) SG=(!(a|c))||(!(b|d));
		else
		{
			si mex;
			if (!a) mex.ins(get(0,1,c,d,l-1));
			if (!b) mex.ins(get(1,0,c,d,l-1));
			if (!c) mex.ins(get(a,b,0,1,l-1));
			if (!d) mex.ins(get(a,b,1,0,l-1));
			ft(x,1,l-1-1)
			{
				int y=l-1-x;
				mex.ins(get(a,b,1,0,x)^get(1,0,c,d,y));
				mex.ins(get(a,b,0,1,x)^get(0,1,c,d,y));
			}
			SG=0;
			whl(mex.count(SG)) SG++;
		}
	}
	rtn sg[mp(mp(mp(a,b),mp(c,d)),l)];
}
...
int main()
{
	...
	rep(i,n)
	{
		p[i]-=mp(1,1);
		g[p[i].x][p[i].y]=1;
		if (p[i].x-1>=0) g[p[i].x-1][1-p[i].y]=1;
		g[p[i].x][1-p[i].y]=1;
		if (p[i].x+1<r) g[p[i].x+1][1-p[i].y]=1;
	}
	vi c;
	c.pb(-1);
	rep(i,r) if (g[i][0]&&g[i][1]) c.pb(i);
	c.pb(r);
	int ans=0;
	rep(i,sz(c)-1) if (c[i]+1!=c[i+1])
	{
		ans^=get(g[c[i]+1][0],g[c[i]+1][1],g[c[i+1]-1][0],g[c[i+1]-1][1],c[i+1]-c[i]-1);
	}
