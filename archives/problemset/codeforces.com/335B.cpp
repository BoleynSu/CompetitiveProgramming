...
int main()
{
	...
	mci cnt;
	rep(i,sz(s)) cnt[s[i]]++;
	ft(i,'a','z') if (cnt[i]>=100)
	{
		rep(j,100) cout<<char(i);
		cout<<endl;
		rtn 0;
	}
	vvi f(sz(s),vi(sz(s)));
	rep(i,sz(s)) f[i][i]=(s[i]==s[i]?1:0);
	repf(i,1,sz(s)) f[i-1][i]=(s[i-1]==s[i]?2:1);
	repf(l,2,sz(s)) rep(i,sz(s)-l)
	{
		int j=i+l;
		if (s[i]==s[j]) cmax(f[i][j],f[i+1][j-1]+2);
		cmax(f[i][j],f[i+1][j]),cmax(f[i][j],f[i][j-1]);
	}
	str ans;
	int l=0,r=sz(s)-1;
	lp
	{
		...
		else if (s[l]==s[r]&&f[l][r]==f[l+1][r-1]+2) ans.pb(s[l]),l++,r--;
		else if (f[l][r]==f[l+1][r]) l++;
		else if (f[l][r]==f[l][r-1]) r--;
	}
}
