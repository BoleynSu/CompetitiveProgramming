...
pr<lli,pll> calc(pr<lli,pll> a,pr<lli,pll> b)
{
	pr<lli,pll> c;
	c.x=a.x+b.x;
	c.y.x=a.y.x;
	c.y.y=a.y.y+b.y.y;
	c.x+=(b.y.x-a.y.x)*b.y.y;
	rtn c;
}
//最少次数 当前和 最小减去
pr<lli,pll> dfs(int u,int p=-1)
{
	pr<lli,pll> ans=mp(0ll,mp(0ll,1ll));
	vec<pr<lli,pll> > lst;
	bool lv=true;
	for (int v:adj[u]) if (v!=p)
	{
		lv=false;
		pr<lli,pll> get=dfs(v,u);
		lst.pb(get);
	}
	lli l=1;
	rep(i,sz(lst))
	...
	ans.y.y=min(lli(+oo),l*sz(lst));
	ans.x=0;
	lli min=+oo;
	rep(i,sz(lst)) cmin(min,lst[i].y.x/l);
	rep(i,sz(lst)) ans.x+=lst[i].x+lst[i].y.x-l*min;
	ans.y.x=l*min*sz(lst);
	if (lv) ans=mp(0ll,mp(a[u],1ll));
	prt(mp(u,ans));
	rtn ans;
}
