...
int main()
{
	...
	int s=a%magic;//s->0;
	prt(s);
	lli cir=a/magic-b/magic;//magic-1->0;
	prt(cir);
	int t=b%magic;//magic-1->t;
	prt(t);
	...
	d=vec<lli>(magic,+ooll);
	if (cmin(d[s],0ll)) q.push(s);
	whl(sz(q))
	{
		int u=q.front();
		q.pop();
	...
	prt(ans);
	d=vec<lli>(magic,+oo);
	if (cmin(d[magic-1],0ll)) q.push(magic-1);
	whl(sz(q))
	{
		int u=q.front();
		q.pop();
	...
	prt(magic);
	ans+=lli(d[0]+1)*(cir-1);//magic-1->0->magic-1
	prt(d[0]+1);
	ans+=d[t]+1;
	if (cir) cout<<ans<<endl;
	else cout<<sp<<endl;
}
