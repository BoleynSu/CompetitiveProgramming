...
int main()
{
    ...
    vi h(n+1);
    ft(i,1,n) cin>>h[i];
    const int oo=1993039;
    vec<vec<pii> > f(n+1,vec<pii>(a+1,mp(oo,oo)));
    f[0][0]=mp(0,0);
    int sum=0;
    ft(i,1,n)
    {
        ...
        fdt(j,min(a,sum),0)
        {
            if (j>=h[i])
            {
                cmin(f[i][j].x,f[i-1][j-h[i]].y+min(h[i-1],h[i]));
                cmin(f[i][j].x,f[i-1][j-h[i]].x);
            }
            if (sum-j<=b)
            {
                cmin(f[i][j].y,f[i-1][j].x+min(h[i-1],h[i]));
                cmin(f[i][j].y,f[i-1][j].y);
            }
        }
    }
    int ans=oo;
    ft(i,0,a) cmin(ans,f[n][i].x),cmin(ans,f[n][i].y);
    if (ans==oo) puts("-1");
    else cout<<ans<<endl;
}
