...
node ch[(MAXN<<1)-1],an[(MAXN<<1)-1];
void upd(node st[],int l,int r,int L,int R,int v)
{
	if (R<l||r<L) ;
	else if (L<=l&&r<=R)
	{
	...
	else
	{
		...
		{
			st[lrt].lz=st[rt].lz;
			st[lrt].max=st[rt].max;
			st[rrt].lz=st[rt].lz;
			st[rrt].max=st[rt].max;
			st[rt].lz=false;
		}
		upd(st,l,m,L,R,v),upd(st,m+1,r,L,R,v);
		cmax(st[rt].max,st[lrt].max);
		cmax(st[rt].max,st[rrt].max);
	}
}
int qry(node st[],int l,int r,int L,int R)
{
	if (R<l||r<L)
	{
		rtn -oo;
	}
	else if (L<=l&&r<=R) rtn st[rt].max;
	else
	{
		int m=(l+r)>>1;
		if (st[rt].lz)
		...
		rtn max(qry(st,l,m,L,R),qry(st,m+1,r,L,R));
	}
}
...
int main()
{
	...
	dfs(0);
	upd(ch,0,n-1,in[0],out[0]-1,-2);
	upd(an,0,n-1,in[0],out[0]-1,-1);
	int q;
	cin>>q;
	rep(i,q)
	{
		...
		if (c==1)
		{
			upd(ch,0,n-1,in[v],out[v]-1,i);
		}
		if (c==2)
		{
			upd(an,0,n-1,in[v],in[v],i);
		}
		if (c==3)
		{
			cout<<(qry(ch,0,n-1,in[v],in[v])>qry(an,0,n-1,in[v],out[v]-1))<<endl;
		}
	}
}
