...
int main()
{
	...
	repf(i,1,sz(v))
	{
		whl(j!=-1&&v[j+1]!=v[i]) j=pi[j];
		if (v[j+1]==v[i]) j++;
		pi[i]=(j!=-1&&i+1<sz(v)&&v[j+1]==v[i+1])?pi[j]:j;
	}
	ft(i,0,sz(s1)) ft(j,0,sz(s2)) rep(k,sz(v)) f[i][j][k]=-oo;
	f[0][0][0]=0;
	int ans=-oo,ansi,ansj,ansk;
	rep(i,sz(s1)) rep(j,sz(s2)) rep(k,sz(v))
	{
		if (s1[i]==s2[j])
		{
			int nk=k-1;
			whl(nk!=-1&&s1[i]!=v[nk+1]) nk=pi[nk];
			if (v[nk+1]==s1[i]) nk++;
			if (i+1==4&&j+1==4) prt(mp(k,nk));
			if (nk!=sz(v)-1&&cmax(f[i+1][j+1][nk+1],f[i][j][k]+1)) g[i+1][j+1][nk+1]=mp(mp(i,j),k);
			if (cmax(f[i+1][j+1][k],f[i][j][k])) g[i+1][j+1][k]=mp(mp(i,j),k);
		}
		else
		{
			if (cmax(f[i+1][j][k],f[i][j][k])) g[i+1][j][k]=mp(mp(i,j),k);
			if (cmax(f[i][j+1][k],f[i][j][k])) g[i][j+1][k]=mp(mp(i,j),k);
		}
	}
	ft(i,0,sz(s1)) ft(j,0,sz(s2)) rep(k,sz(v)) if (cmax(ans,f[i][j][k])) ansi=i,ansj=j,ansk=k;
	//prt(ans),prt(ansi),prt(ansj),prt(ansk);
	str anss;
	whl(ansi||ansj)
	{
		prt(g[ansi][ansj][ansk]),prt(ansk);
		int nansi=g[ansi][ansj][ansk].x.x;
		int nansj=g[ansi][ansj][ansk].x.y;
		int nansk=g[ansi][ansj][ansk].y;
		if (ansi!=nansi&&ansj!=nansj&&f[ansi][ansj][ansk]!=f[nansi][nansj][nansk])
			anss.pb(s1[ansi-1]),prt(s1[ansi-1]);
		ansi=nansi,ansj=nansj,ansk=nansk;
	}
	rev(anss);
	if (sz(anss)) cout<<anss<<endl;
	else cout<<0<<endl;
}
//int main()
//{
