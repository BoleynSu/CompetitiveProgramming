...
int main()
{
	...
	pr<lli,pii> ans=mp(-ooll,mp(0ll,0ll));
	pr<lli,int> a=mp(-ooll,0ll);
	ft(i,2*k,n)
	{
		cmax(a,mp(sum[i-k]-sum[(i-k)-k],-((i-k)-k+1)));
		pr<lli,int> b=pll(sum[i]-sum[i-k],-(i-k+1));
		cmax(ans,mp(a.x+b.x,mp(a.y,b.y)));
	}
