...
time_t toTime(cstr st)
{
    int Y,M,D,h,m,s;
    sscanf(st,"%d-%d-%d %d:%d:%d",&Y,&M,&D,&h,&m,&s);
    tm t;
...
int main()
{
    ...
    lp
    {
        if (!gets(s)) break;
        time_t t=toTime(s);
        whl(sz(q)&&q.front()+n<=t) q.pop_front();
        q.push_back(t);
        if (sz(q)>=m)
        {
            rep(i,19) putchar(s[i]);
            putchar('\n');
            return 0;
        }
    }
...
//  ft(i,1,(n-1)/2)
//  {
//      whl(a[i]>0||(i*2>(n-1)/2&&a[i*2]>0)||(i*2+1>(n-1)/2&&a[i*2+1]>0))
//      {
//          a[i]--,a[i*2]--,a[i*2+1]--;
//          ans++;
//      }
//  }
//  ft(i,1,n) if (a[i]>0) ans=-1;
//  cout<<ans<<endl;
}
