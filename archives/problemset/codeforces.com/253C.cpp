...
map<pii,lli> d;
prq<pr<lli,pii>,vec<pr<lli,pii> >,greater<pr<lli,pii> > > q;
void go(int x,int y,pii u,lli delta)
{
    pii v=mp(x,y);
    if (!d.count(v)) d[v]=+ooll;
    if (cmin(d[v],d[u]+delta)) q.push(mp(d[v],v));
}
...
int main()
{
    ...
    si ys;
    ys.insert(0);
    rep(i,n) ys.insert(a[i]);
    ys.insert(s.y),ys.insert(t.y);
    q.push(mp(d[s]=0,s));
    whl(sz(q))
    {
        pii u=q.top().y;
        ...
        {
            int x=u.x,y=u.y;
            if (x>0) go(x-1,min(y,a[x-1]),u,1);
            if (x<n-1) go(x+1,min(y,a[x+1]),u,1);
            feach(it,ys) if (*it<=a[x]) go(x,*it,u,abs(*it-y));
        }
    }
    cout<<d[t]<<endl;
}
