...
pr<int,pii> dfs(pii d,int u,int p=-1)
{
	pr<int,pii> ans=mp(oo,mp(0,d.y-d.x*2));
	rep(i,sz(adj[u]))
	{
		int v=adj[u][i];
		if (v==p) continue;
		pr<int,pii>  get=dfs(mp(d.x+adjd[u][i],d.y+1),v,u);
		cmin(ans.x,get.x);
		ans.y.x+=get.y.x+!adjd[u][i];
		cmax(ans.y.y,get.y.y);
	}
	cmin(ans.x,(d.y-d.x*2)-ans.y.y);
	rtn ans;
}
int calc(pr<int,pii> get)
{
	rtn get.x+get.y.x;
}
...
int main()
{
	...
	repf(i,1,n)
	{
		int ai,bi;
		cin>>ai>>bi,--ai,--bi;
		adj[ai].pb(bi),adj[bi].pb(ai);
		adjd[ai].pb(1),adjd[bi].pb(0);
	}
	int ans=oo;
	rep(i,n) cmin(ans,calc(dfs(mp(0,0),i)));
	cout<<ans<<endl;
}
