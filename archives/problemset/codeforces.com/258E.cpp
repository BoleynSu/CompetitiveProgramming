...
#define lc idx(l,m)
#define rc idx(m+1,r)
void insert(int l,int r,int L,int R,int v)
{
	if (R<l||r<L) ;
	else if (L<=l&&r<=R)
	{
		int m=(l+r)>>1;
		st[p].x+=v;
		if (st[p].x) st[p].y=r-l+1;
		else if (l<r) st[p].y=st[lc].y+st[rc].y;
		else st[p].y=0;
	}
	else
	{
		int m=(l+r)>>1;
		insert(l,m,L,R,v),insert(m+1,r,L,R,v);
		if (st[p].x) st[p].y=r-l+1;
		else st[p].y=st[lc].y+st[rc].y;
	}
}
...
{
	rep(i,sz(qry[u])) insert(0,ord-1,rng[qry[u][i]].x,rng[qry[u][i]].y,+1);
	ans[u]=query(0,ord-1);
	rep(i,sz(adj[u]))
	{
		int v=adj[u][i];
...
int main()
{
	...
	rep(i,n) if (sz(qry[i])) qry[i].pb(i);
	calc(0,-1);
	rep(i,n) pf("%d%c",max(ans[i]-1,0),char(i+1==n?'\n':' '));
}
