...
int main()
{
	...
	vi c,d;
	rep(i,sz(a))
		if (a[i].x=="ATK") c.pb(a[i].y);
		else d.pb(a[i].y);
	srt(c),srt(d);
	rev(c),rev(d);
	int ans=0;
	vvi f(sz(b)+1,vi(sz(c)+1,-oo));
	vec<vvi> g(sz(b)+1,vvi(sz(c)+1,vi(sz(d)+1,-oo)));
	f[0][0]=0;
	g[0][0][0]=0;
	ft(i,0,sz(b)) ft(j,0,sz(c))
	{
		if (i) cmax(f[i][j],f[i-1][j]);
		if (j) cmax(f[i][j],f[i][j-1]);
		if (i&&j&&b[i-1]>=c[j-1])
			cmax(f[i][j],f[i-1][j-1]+b[i-1]-c[j-1]);
	}
	ft(i,0,sz(b)) ft(j,0,sz(c)) ft(k,0,sz(d))
	{
		if (i) cmax(g[i][j][k],g[i-1][j][k]+b[i-1]);
		if (i&&j&&b[i-1]>=c[j-1])
			cmax(g[i][j][k],g[i-1][j-1][k]+b[i-1]-c[j-1]);
		if (i&&k&&b[i-1]>d[k-1])
			cmax(g[i][j][k],g[i-1][j][k-1]);
	}
	cmax(ans,f[sz(b)][sz(c)]);
	cmax(ans,g[sz(b)][sz(c)][sz(d)]);
	cout<<ans<<endl;
}
