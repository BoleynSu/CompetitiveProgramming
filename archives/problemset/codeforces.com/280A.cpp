...
int main()
{
	db w,h,alpha;
	cin>>w>>h>>alpha;
	vec<Halfplane> hp,ahp;
	Polygon ch;
	w/=2,h/=2,alpha/=180,alpha*=pi;
	Point d(-w,h),c(w,h),b(w,-h),a(-w,-h);
	hp.pb(mp(a,b));
	hp.pb(mp(b,c));
	hp.pb(mp(c,d));
	hp.pb(mp(d,a));
	...
	halfplane_intersection(ch,ahp,hp);
	db s=0;
	for (int i=0,j=sz(ch)-1;i<sz(ch);j=i++)
	{
		prt(ch[i]);
		s+=cross(ch[i],ch[j]);
	}
