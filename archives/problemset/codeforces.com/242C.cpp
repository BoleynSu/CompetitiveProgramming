...
const int ds=8;
const int dx[ds]={-1,-1,-1,0,0,1,1,1};
const int dy[ds]={-1,0,1,-1,1,-1,0,1};
...
int main()
{
	...
	rep(i,n)
	{
		int r,a,b;
		scanf("%d%d%d",&r,&a,&b);
		ft(i,a,b) ps[pss++]=(mp(r,i));
	}
	sort(ps,ps+pss);
	pss=unique(ps,ps+pss)-ps;
	int s=find(ps,ps+pss,mp(x0,y0))-ps,t=find(ps,ps+pss,mp(x1,y1))-ps;
	q[qh]=s;
	d[s]=1;
	whl(qh<=qt)
	{
		...
		rep(i,ds)
		{
			pii p=mp(ps[x].x+dx[i],ps[x].y+dy[i]);
			int y=lower_bound(ps,ps+pss,p)-ps;
			if (y<pss&&ps[y]==p&&!d[y])
			{
				if (y==t)
				{
