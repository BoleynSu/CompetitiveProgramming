...
int main()
{
	...
	srt(a);
	int mi=min(a[0].x+a[1].x+h,n<=n?a[1].x+a[2].x:oo);
	int ma=max(a.rbegin()[0].x+a.rbegin()[1].x,a.rbegin()[0].x+a[0].x+h);
	if ((a.rbegin()[0].x+a.rbegin()[1].x)-(a[0].x+a[1].x)<ma-mi)
	{
		cout<<(a.rbegin()[0].x+a.rbegin()[1].x)-(a[0].x+a[1].x)<<endl;
		rep(i,n) cout<<1<<char(i+1==n?'\n':' ');
		rtn 0;
	}
	vi b(n);
	rep(i,n) b[a[i].y]=i?2:1;
	cout<<ma-mi<<endl;
	rep(i,n) cout<<b[i]<<char(i+1==n?'\n':' ');
}
