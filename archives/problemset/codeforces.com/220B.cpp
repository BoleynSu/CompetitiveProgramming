...
int main()
{
	...
	vi p(n+1);
	vi b;
	ft(i,1,n) if (c[i]>=i) p[i]=sz(b),b.pb(i);

	vvi d(sz(b));
	rep(i,n) if (a[i]<=n&&c[a[i]]>=a[i]) d[p[a[i]]].pb(i);
	rep(i,sz(b)) d[i].pb(n);
	...
	rep(q,m)
	{
		...
		rep(i,sz(b))
		{
			int L=lower_bound(d[i].begin(),d[i].end(),l-1)-d[i].begin();
			if (L+b[i]<sz(d[i])&&d[i][L+b[i]]>=r&&d[i][L+b[i]-1]<r)
				ans++;
		}
		cout<<ans<<endl;
	}
