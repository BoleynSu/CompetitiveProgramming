...
int main()
{
	...
	rep(i,n)
	{
		rep(j,m) if (tab[i][j]=='1') pst[i].pb(j-m);
		rep(j,m) if (tab[i][j]=='1') pst[i].pb(j);
		rep(j,m) if (tab[i][j]=='1') pst[i].pb(j+m);
		if (pst[i].empty()) rtn cout<<-1<<endl,0;
	}
	vi ans(m);
	vi cur(n,0);
	rep(i,m)
	{
		rep(j,n)
		{
			whl(pst[j][cur[j]]<i) cur[j]++;
			ans[i]+=min(pst[j][cur[j]]-i,i-pst[j][cur[j]-1]);
			//if (i==0) prt(ans[i]);
		}
	}
