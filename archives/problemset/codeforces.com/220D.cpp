...
//template<typename type>inline type gcd(type a,type b){if (b) while ((a%=b)&&(b%=a));rtn a+b;}

int _gcd[4000+2][4000+2];

int main()
{
	int w,h;
	cin>>w>>h;
	lli wc[2]={w/2+1,w-w/2};
	lli hc[2]={h/2+1,h-h/2};
	int ans=0;
	enum{MOD=1000000007};
	rep(x1,2) rep(y1,2) rep(x2,2) rep(y2,2) rep(x3,2) rep(y3,2)
		if((x2-x1)*(y3-y1)-(y2-y1)*(x3-x1)%2==0)
			ans=(ans+((((wc[x1]*hc[y1])%MOD)*((wc[x2]*hc[y2])%MOD))%MOD)*((wc[x3]*hc[y3])%MOD))%MOD;
	lli s=((w+1)*(h+1))%MOD;
	ans=(ans-s)%MOD;
	if (ans<0) ans+=MOD;
	ans=(ans-(s*(s-1)*3)%MOD)%MOD;
	if (ans<0) ans+=MOD;
	ft(a,0,w)
		ft(b,0,h)
		{
			if (a==0) _gcd[a][b]=b;
			else if (b==0) _gcd[a][b]=a;
			else if (a<=b) _gcd[a][b]=_gcd[a][b-a];
			else _gcd[a][b]=_gcd[a-b][b];
		}
	ft(x,0,w)
		ft(y,0,h)
			if (_gcd[x][y])
			{
				ans=(ans-(lli(_gcd[x][y]-1)*lli(w+1-x)*lli(h+1-y)*lli((x&&y)?12:6))%MOD)%MOD;
				if (ans<0) ans+=MOD;
			}
	cout<<ans<<endl;
}
