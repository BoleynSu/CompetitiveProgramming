...
int main()
{
	int n,m;
	cin>>n>>m;
	vec<vec<char> > map(n,vec<char>(m));
	rep(i,n) rep(j,m) cin>>map[i][j];

	char set[]={'*','.'};
	int ans=0;
	vec<vec<char> > backup=map;
	rep(xy,2) rep(xiy,2) rep(xyi,2) rep(xiyi,2)
	{
		map=backup;
		for (int i=1;i<=min(n,m);i<<=1)
		{
			fdt(x,n-1,0) fdt(y,m-1,0)
					if (x<i*2-1||y<i*2-1) map[x][y]=0;
					else
					{
						if (map[x][y]==set[xy]&&map[x-i][y]==set[xiy]&&map[x][y-i]==set[xyi]&&map[x-i][y-i]==set[xiyi]&&i>1) ans++;
						if (map[x][y]=='*'&&map[x-i][y]=='*'&&map[x][y-i]=='*'&&map[x-i][y-i]=='*') map[x][y]='*';
						else if (map[x][y]==set[xy]&&map[x-i][y]==set[xiy]&&map[x][y-i]==set[xyi]&&map[x-i][y-i]==set[xiyi]) map[x][y]='.';
						else map[x][y]=0;
					}
		}
	}
