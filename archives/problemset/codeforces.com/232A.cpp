...
{
	rep(i,size) rep(j,size) m[start+i][start+j]=(i!=j);
}

int main()
{
	rep(i,1000) c[i][0]=1;
	repf(i,1,1000) repf(j,1,1000) c[i][j]=c[i-1][j]+c[i-1][j-1];
	int k;
	cin>>k;
	fdt(i,100,3)
	{
		if (c[i][3]<=k)
		{
			...
			fdt(j,i,2)
			{
				...
				{
					k-=c[j][2];
					rep(i,j) m[ms+t][i]=m[i][ms+t]=true;
					t++;
				}
			}
			ms+=t;
	...
	cout<<ms<<endl;
	rep(i,ms) rep(j,ms) cout<<m[i][j]<<(j+1==ms?"\n":"");
}
