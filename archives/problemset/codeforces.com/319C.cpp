...
//			//0 xor  1 > 1 xor 1
//			// yes
//			ans=mod(ans*2+delta*delta);
//		}
//		delta=mod(delta+delta);
//	}
//	cout<<ans<<endl;
//}
...
int main()
{
	...
	repf(i,1,n)
	{
		#define X(k,j) (f[j]-f[k])
		#define Y(k,j) (b[k]-b[j])
		whl(qh<qt&&X(q[qh],q[qh+1])/Y(q[qh],q[qh+1])<=a[i]) qh++;
		f[i]=f[q[qh]]+a[i]*b[q[qh]];
//		f[i]=+ooll;
//		rep(j,i)
//			cmin(f[i],f[j]+a[i]*b[j]);
		whl(qh<qt&&X(q[qt-1],q[qt])/Y(q[qt-1],q[qt])>=X(q[qt],i)/Y(q[qt],i)) qt--;
		q[++qt]=i;
	}
	cout<<f.back()<<endl;
}
