...
int main()
{
	...
	rep(i,m)
	{
		...
		cin>>a>>b;
		int aid,bid;
		if (encode.count(a)) aid=encode[a];
		else encode.insert(mp(a,aid=sz(encode))),decode.pb(a),adj.pb(vi());
		if (encode.count(b)) bid=encode[b];
		else encode.insert(mp(b,bid=sz(encode))),decode.pb(b),adj.pb(vi());
		adj[aid].pb(bid);
		adj[bid].pb(aid);
	}
	cout<<sz(adj)<<endl;
	rep(i,sz(adj))
	{
		...
		vi cnt(sz(adj));
		rep(j,sz(adj[i]))
			rep(k,sz(adj[adj[i][j]]))
			cnt[adj[adj[i][j]][k]]++;
		rep(j,sz(adj[i])) cnt[adj[i][j]]=-oo;
		cnt[i]=-oo;
		int max=0,ans=0;
		rep(i,sz(cnt))
		{
			if (cmax(max,cnt[i])) ans=0;
			if (max==cnt[i]) ans++;
		}
		cout<<ans<<endl;
	}
//	rep(i,sz(adj)) srt(adj[i]);
//	vvi f(sz(adj),vi(sz(adj)));
//	rep(i,sz(adj))
//	{
//		rep(j,i) if (!binary_search(all(adj[i]),j))
//		{
//			vi common(sz(adj[i]));
//			common.erase(set_intersection(all(adj[i]),all(adj[j]),common.begin()),common.end());
//			f[i][j]=f[j][i]=sz(common);
//		}
//		else f[i][j]=f[j][i]=-oo;
//		f[i][i]=-oo;
//	}
...
//		rep(j,sz(adj))
//		{
//			if (cmax(max,f[i][j])) cnt=0;
//			if (max==f[i][j]) cnt++;
//		}
//		cout<<decode[i]<<" "<<cnt<<endl;
//	}
}
