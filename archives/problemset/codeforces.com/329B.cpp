...
int main()
{
	int r,c;
	cin>>r>>c;
	vec<vec<char> > g(r,vec<char>(c));
	cin>>g;
	pii exit;
	rep(i,r) rep(j,c) if (g[i][j]=='E') exit=mp(i,j);
	queue<pii> q;
	vvi d(r,vi(c,+oo));
	if (cmin(d[exit.x][exit.y],0)) q.push(exit);
	whl(sz(q))
	{
		pii u=q.front();
		q.pop();
		rep(dir,4)
		{
			pii v=u+mp(dx[dir],dy[dir]);
			if (v.x>=0&&v.y>=0&&v.x<r&&v.y<c&&g[v.x][v.y]!='T'&&cmin(d[v.x][v.y],d[u.x][u.y]+1))
				q.push(v);
		}
	}
	int maxd;
	rep(i,r) rep(j,c) if (g[i][j]=='S') maxd=d[i][j];
	int ans=0;
	rep(i,r) rep(j,c) if (isdigit(g[i][j])&&d[i][j]<=maxd) ans+=g[i][j]-'0';
	cout<<ans<<endl;
}
