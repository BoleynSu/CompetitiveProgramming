...
int main()
{
	...
	cin>>g;
	{
		build_graph(n*2,n*2+1);
		vec<vec<edge> > e(n,vec<edge>(n));
		rep(i,n) add_edge(S,i,n);
		rep(i,n) add_edge(i+n,T,1);
		rep(i,n) rep(j,n)
		{
			e[i][j]=top+1;
			add_edge(i,j+n,g[i][j]=='.');
			prt(e[i][j]);
		}
		if (dinic()==n)
		{
			rep(i,n) rep(j,n) if (e[i][j]->c) cout<<i+1<<" "<<j+1<<endl;
			rtn 0;
		}
	}
	{
