...
int main()
{
	...
	int s=0;
	feach(i,f,mii) i->second=s++;
	vi lbst(s),rbst(s);
	lli sum=0,ans=0;
	fdt(i,n-1,1)
	{
		sum+=bit_sum(rbst,f[a[i]]-1);
		bit_inc(rbst,f[a[i]],1);
	}
	int r=1;
	rep(l,n)
	{
		int lc=bit_sum(lbst,s-1)-bit_sum(lbst,f[a[l]]);
		int rc=bit_sum(rbst,f[a[l]]-1);
		bit_inc(lbst,f[a[l]],1);
		sum+=lc+rc;
		while (r<=l||(r<n&&sum>k))
		{
			int lc=bit_sum(lbst,s-1)-bit_sum(lbst,f[a[r]]);
			int rc=bit_sum(rbst,f[a[r]]-1);
			bit_inc(rbst,f[a[r]],-1);
			sum-=lc+rc;
			r++;
		}
		if (sum<=k) ans+=n-r;
