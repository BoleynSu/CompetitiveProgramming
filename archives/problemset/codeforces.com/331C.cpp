...
//			x/=10;
//		}
//		if (f[i-1]>f[i]) prt(i),prt(f[i]),prt(f[i-1]);
//	}
...
int main()
{
	repf(i,1,1<<20) maxd[i]=max(maxd[i/10],i%10);
	rep(maxhd,10)
	{
		int* f=::f[maxhd];
		int* g=::g[maxhd];
		repf(i,1,1<<20)
		{
			int delta=max(maxd[i],maxhd);
			if (i>delta) f[i]=f[i-delta]+1,g[i]=g[i-delta];
			else f[i]=0+1,g[i]=i-delta;
		}
	}
	lli n;
	cin>>n;
