...
void dfs(int n,vec<lli>& a,vi& cnt,int i,vec<lli>& c,int& ans,int ttl,int nf)
{
	if (i==n)
	{
		//prt(ttl),prt(c);
	...
	else
	{
		rep(j,n) if (i!=j)
		{
			...
			{
				c[j]*=a[i];
				dfs(n,a,cnt,i+1,c,ans,ttl-cnt[i],nf-1);
				c[j]/=a[i];
			}
		}
		dfs(n,a,cnt,i+1,c,ans,ttl,nf);
	}
}
...
int main()
{
	...
	int ans=1;
	rep(i,n) ans+=cnt[i]==1?1:cnt[i]+1;
	vec<lli> c(n,1);
	dfs(n,a,cnt,0,c,ans,ans,n);
	cout<<ans<<endl;
}
