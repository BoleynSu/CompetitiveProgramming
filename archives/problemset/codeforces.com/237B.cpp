...
int main()
{
	...
	rep(i,n) rep(j,c[i])
	{
		pii minp=pii(i,j);
		int min=d[i][j];
		rep(ii,n) rep(jj,c[ii])
		{
			if (pii(ii,jj)>pii(i,j))
			{
				if (cmin(min,d[ii][jj])) minp=pii(ii,jj);
			}
		}
		if (minp!=pii(i,j))
		{
			ans.pb(mp(pii(i,j),minp));
			swap(d[i][j],d[minp.x][minp.y]);
		}
	}
	cout<<sz(ans)<<endl;
	rep(i,sz(ans)) cout<<ans[i].x.x+1<<" "<<ans[i].x.y+1<<" "<<ans[i].y.x+1<<" "<<ans[i].y.y+1<<endl;
}
