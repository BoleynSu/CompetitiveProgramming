...
int main()
{
	...
	lli ans=-oo,ansx=-1000000000,ansy=-1000000000;
	ft(x,-max(na,nb),max(na,nb)) ft(y,-max(ma,mb),max(ma,mb))
	{
		lli get=0;
		rep(i,na) rep(j,ma) if (0<=i+x&&i+x<nb&&0<=j+y&&j+y<mb) get+=a[i][j]*b[i+x][j+y];
		if (cmax(ans,get)) ansx=x,ansy=y;
	}
	cout<<ansx<<" "<<ansy<<endl;
}
