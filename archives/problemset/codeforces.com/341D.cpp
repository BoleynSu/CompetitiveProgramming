...
const int MAXN=1000;
lli c[2][2][MAXN+2][MAXN+2];
void upd(lli c[MAXN+2][MAXN+2],int x,int y,lli v)
{
	for (int i=x;i<=MAXN;i+=i&(-i))
		for (int j=y;j<=MAXN;j+=j&(-j))
			c[i][j]^=v;
}
lli qry(lli c[MAXN+2][MAXN+2],int x,int y)
{
	lli ans=0;
	for (int i=x;i;i-=i&(-i))
		for (int j=y;j;j-=j&(-j))
...
void put(int x0,int y0,int x1,int y1,lli v)
{
	int i0=x0&1,j0=y0&1,i1=x1&1,j1=y1&1;
	upd(c[i0][j0],x0,y0,v);
	upd(c[!i1][j0],x1+1,y0,v);
	upd(c[i0][!j1],x0,y1+1,v);
	upd(c[!i1][!j1],x1+1,y1+1,v);
}
lli get(int x0,int y0,int x1,int y1)
{
	lli ans=0;
	int i0=x0&1,j0=y0&1,i1=x1&1,j1=y1&1;
	ans^=qry(c[!i0][!j0],x0-1,y0-1);
	ans^=qry(c[i1][!j0],x1,y0-1);
	ans^=qry(c[!i0][j1],x0-1,y1);
	ans^=qry(c[i1][j1],x1,y1);
	rtn ans;
}
