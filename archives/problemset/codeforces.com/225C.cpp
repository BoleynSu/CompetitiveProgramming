...
int main()
{
    ...
    vvi f[2]={vvi(m+1,vi(y+1,+oo)),vvi(m+1,vi(y+1,+oo))};
    f[0][0][y]=f[1][0][y]=0;
    ft(i,1,m)
        rep(k,2)
        {
        //prt(c[i]);
            ft(j,1,y-1) cmin(f[k][i][j+1],f[k][i-1][j]+(k?n-c[i]:c[i]));
            ft(j,x,y) cmin(f[k][i][1],f[!k][i-1][j]+(k?n-c[i]:c[i]));
            //ft(j,0,y) cout<<i<<" "<<j<<" "<<k<<":"<<f[k][i][j]<<endl;
        }
    int ans=+oo;
    rep(k,2) ft(i,x,y) cmin(ans,f[k][m][i]);
    cout<<ans<<endl;
}
