...
lli ans[MAXN];
lli lz[MAXN];
lli cnt[100000+2][316+2];

int main()
{
	sf("%d%d%d",&n,&m,&q);
	ft(i,1,n) sf("%I64d",&a[i]);
	int limit=sqrt(n)+1;
	ft(i,1,m)
	{
		sf("%d",&k[i]);
		S[i].resize(k[i]);
		rep(j,k[i]) sf("%d",&S[i][j]);
		srt(S[i]);
		if (k[i]>limit) id[i]=sz(large),large.pb(i);
	}
	ft(i,1,n)
	{
		rep(x,k[i])
		{
			rep(y,sz(large)) if (binary_search(all(S[large[y]]),S[i][x])) cnt[i][y]++;
		}
	}
	ft(i,1,n)
	{
		rep(x,sz(large)) if (binary_search(all(S[large[x]]),i)) ans[large[x]]+=a[i];
	}
	rep(it,q)
	{
		char cmd;
		...
		if (cmd=='?')
		{
			...
			{
				ans[i]=0;
				rep(j,k[i]) ans[i]+=a[S[i][j]];
				rep(j,sz(large)) ans[i]+=cnt[i][j]*lz[large[j]];
			}
			pf("%I64d\n",ans[i]);
		}
		else
		{
			...
			{
				rep(j,k[i]) a[S[i][j]]+=x;
				rep(j,sz(large)) ans[large[j]]+=cnt[i][j]*x;
			}
			else
			{
				lz[i]+=x;
			...
			//ft(i,1,n) cerr<<i<<":"<<a[i]<<" "<<lz[i]<<" "<<ans[i]<<endl;
		}
	}
}
