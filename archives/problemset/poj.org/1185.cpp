...
int main()
{
	...
	{
		ms[i]=0;
		rep(j,m) ms[i]<<=1,ms[i]|=map[i][j]=='H';
	}
	rep(i,ss) if (!(ms[0]&s[i])) f[0][i][0]=sc[i];
	repf(i,1,n)
		rep(j,ss)
			rep(k,ss)
				rep(l,ss)
					if (!((s[j]&s[k])||(s[j]&s[l])||(s[j]&ms[i])))
						cmax(f[i][j][k],f[i-1][k][l]+sc[j]);
	rep(i,ss) rep(j,ss) cmax(ans,f[n-1][i][j]);
	cout<<ans<<endl;
}