...
int main()
{
	...
	whl(cin>>a>>b)
	{if (!(a&&b)) break;
		BigInteger ansx=1,ansy=1;
		rep(i,b) ansx=ansx*(a-i);
		rep(i,b) ansy=ansy*(i+1);
		cout<<a<<" things taken "<<b<<" at a time is "<<ansx/ansy<<" exactly.\n";
		/*try
		{
			prt(a<b),prt(a==b),prt(a+b),prt(a-b),prt(a*b),prt(a/b),prt(a%b),prt(sqrt(a)),prt(sqrt(b));
		}
		catch (const char* x)
		{
			prt(x);
