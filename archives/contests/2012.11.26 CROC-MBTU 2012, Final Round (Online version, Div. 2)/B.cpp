...
vs get(str ip)
{
	...
	repf(i,1,sz(ip))
	{
		if (ip[i-1]==':'&&ip[i]==':')
		{
			vs a=get(ip.substr(0,i-1));
			vs b=get(ip.substr(i+1,sz(ans)-i));
			rep(i,sz(a)) ans.pb(a[i]);
			rep(i,8-sz(a)-sz(b)) ans.pb("0000");
			rep(i,sz(b)) ans.pb(b[i]);
			rtn ans;
		}
	}
	lp
	{
		...
		if (ind!=string::npos)
		{
			ans.pb(ip.substr(0,ind));
			ip=ip.substr(ind+1,sz(ip)-ind-1);
			//prt(ans.back());
			//prt(ip);
		}
		else
...
int main()
{
	...
	rep(i,n)
	{
		...
		rep(j,sz(ans))
		{
			rep(i,4-sz(ans[j])) cout<<"0";
			cout<<ans[j]<<char(j+1==sz(ans)?'\n':':');
		}
	}
}