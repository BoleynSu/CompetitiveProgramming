...
int main()
{
    ...
    rep(i,2) sort(all(lst[i]),greater<pii>());
    vi cur(2);
    int curc=0;
    whl(cur[curc]<sz(lst[curc]))
    {
        if (lst[curc][cur[curc]].x>=lst[!curc][cur[!curc]].x)
        {
            cout<<lst[curc][cur[curc]].y<<" "<<lst[!curc][cur[!curc]].y<<" "<<lst[!curc][cur[!curc]].x<<endl;
            lst[curc][cur[curc]].x-=lst[!curc][cur[!curc]].x;
            if (cur[!curc]+1==sz(lst[!curc])) cur[curc]++;
            else cur[!curc]++;
        }
        else
        {
