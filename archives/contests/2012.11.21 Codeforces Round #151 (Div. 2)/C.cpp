...
int main()
{
    ...
    rep(i,n) cin>>a[i];
    map<int,si> f;
    map<int,map<int,pii> > pre;
    f[-1].insert(0);
    rep(i,n)
    {
        feach(it,f[i-1])
        {
            f[i].insert(*it),pre[i][*it]=mp(i-1,*it);
            f[i].insert(*it+a[i]),pre[i][*it+a[i]]=mp(i-1,*it);
            if (sz(f[i])>k) break;
        }
    }
    map<int,si> ans;
    feach(fi,f[n-1])
    {
        ...
        {
            si& ins=ans[*fi];
            for (pii j=mp(n-1,*fi);j.x!=-1;j=pre[j.x][j.y])
            {
                if (j.y-pre[j.x][j.y].y>0) ins.insert(j.y-pre[j.x][j.y].y);
            }
        }
        if (sz(ans)==k) break;
    }
