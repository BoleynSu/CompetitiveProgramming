...
int main()
{
	...
	{
		int ct=0;
		rep(i,K) ct+=mat[i].count(it->x);
		if (ct==K)
		{
			vi endp(K);
			rep(i,K) endp[i]=mat[i][it->x]-1;
			lp
			{
				int maxendp=-oo,maxendpi;
				rep(i,K) if (cmax(maxendp,endp[i])) maxendpi=i;
				bool same=true;
				rep(i,K) if (endp[i]!=maxendp) same=false;
				if (same)
				{
					ans+=(cnt[it->x]=cnt[maxendp]+1);
					break;
				}
				if (mat[maxendpi].count(maxendp)) endp[maxendpi]=mat[maxendpi][maxendp]-1;
				else break;
			}
		}
	}
