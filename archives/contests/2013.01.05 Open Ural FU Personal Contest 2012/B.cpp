...
int main()
{
	...
	rep(i,n)
	{
		...
		whl(x[sz(x)-4]!='.') x.pb('0');
		whl(y[sz(y)-4]!='.') y.pb('0');
		int xx=(x[sz(x)-3]-'0')*100+(x[sz(x)-2]-'0')*10+(x[sz(x)-1]-'0')*1;
		int yy=(y[sz(y)-3]-'0')*100+(y[sz(y)-2]-'0')*10+(y[sz(y)-1]-'0')*1;
		if (x[0]=='-'&&xx) xx=1000-xx;
		if (y[0]=='-'&&yy) yy=1000-yy;
		cnt[xx][yy]++;
	}
	int ansc=-oo;
	db ansd=inf;
	...
	{
		if (cmax(ansc,cnt[i][j])) ansd=inf;
		if (ansc==cnt[i][j]) cmin(ansd,sqrt(db(sqr(min(i,1000-i))+sqr(min(j,1000-j))))/1000);
	}
	cout<<ansc<<" ";
	pdb(10,ansd)<<endl;
}