...
int toInt(const str& x)
{
	rtn int(x[0])<<16|int(x[1])<<8|int(x[2]);
}
...
int main()
{
	...
	getline(cin,a);
	getline(cin,b);
	stringstream ain(a),bin(b);
	str let;
	vi as,bs;
	whl(ain>>let) as.pb(toInt(let));
	whl(bin>>let) bs.pb(toInt(let));
	mii acnt,bcnt;
	rep(i,sz(as)) acnt[as[i]]++;
	int ans=0,matchs=0;
	rep(i,sz(bs))
	{
		if (i-sz(as)>=0&&acnt.count(bs[i-sz(as)]))
		{
			int& bc=bcnt[bs[i-sz(as)]];
			int& ac=acnt[bs[i-sz(as)]];
			if (bc==ac) matchs--;
			--bc;
			if (bc==ac) matchs++;
		}
