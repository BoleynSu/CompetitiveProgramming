...
int main()
{
	...
	rep(i,n)
	{
		...
		rep(i,sz(c))
		{
			...
			rep(j,sz(c[i])) fprintf(file,"%c",c[i][j]?' ':'*');
			fprintf(file,"\n");
		}
		fclose(file);
	}
	...
	rep(i,n)
	{
		...
		rep(j,n)
		{
			int cnt=LRmatch(cs[i],cs[j],height,width);
			if (!cnt) LRcost[i][j]=height*func(255);
			else LRcost[i][j]-=(cnt-1)*func(255)*30;
		}
		rep(j,n)
		{
			int cnt=UDmatch(cs[i],cs[j],height,width);
			if (!cnt) UDcost[i][j]=width*func(255);
			else UDcost[i][j]-=(cnt-1)*func(255)*20;
		}
	}
