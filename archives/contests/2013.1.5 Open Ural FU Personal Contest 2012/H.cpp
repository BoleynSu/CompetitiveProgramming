...
int main()
{
	//f(x+i)=a x^2 + (b+2 a i) x + a i^2+b i+c
	//sum{f(x+i);i=0 to k-1}=akx^2+bkx+ck+a*(k-1)*(2*(k-1)+1)*((k-1)+1)/6+b*(k)*(k-1)/2+2a*(k)*(k-1)/2*x
	//F(x)=a k x^2 + 1/6 k (-6 a + 6 b + 6 a k) x + 1/6 k (a - 3 b + 6 c - 3 a k + 3 b k + 2 a k^2)
	//delta(k)=-a^2/3 k^4 + (a^2/3 + b^2 - 4 a c) k^2
	int t;
	cin>>t;
	whl(t--)
	{
		db a,b,c;
		cin>>a>>b>>c;
		db A=-1.0/3.0*a*a,C=1.0/3.0*a*a+b*b-4.0*a*c;
		cout<<int(sgn(-C/A)<=0?1:ceil(sqrt(-C/A)+eps))<<endl;
	}
}
