...
//O(n)的筛法求素数表
//MAXPS=[1,MAXP]中素数的个数
const int MAXP=77777;
const int MAXPS=77777;
...
struct TheSquareRootDilemma
{
	int countPairs(int N, int M)
	{
		make_prime_table();
		vi tab;
		for (int i=1;sqr(i)<=M;i++) tab.pb(sqr(i));
		lli ans=0;
		for (int i=1;i<=N;i++)
		{
			int x=i,y,z=1;
...
int main()
{
	TheSquareRootDilemma app;
	prt(app.countPairs(77777,77777));
}
#endif
